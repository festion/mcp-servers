...
exports.ClaudeCommitClient = void 0;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
class ClaudeCommitClient {
    client = null;
    auth;
    sessionToken = null;
    constructor(auth) {
    ...
    async performWebAuthentication() {
    ...
    async generateCommitMessage(diffEntries, config = {}) {
        await this.ensureAuthenticated();
        const startTime = Date.now();
        const { language = 'en', conventional_commits = false, include_emoji = false, max_length = 72, template } = config;
        // Analyze the changes
    ...
    analyzeDiff(diffEntries) {
        ...
        const hasTestFiles = diffEntries.some(entry => entry.file.includes('test') || entry.file.includes('spec'));
        const hasDocFiles = diffEntries.some(entry => entry.file.endsWith('.md'));
        // Analyze file extensions to determine languages
        const languages = [...new Set(diffEntries.map(entry => {
        ...
        // Determine impact
        let impact;
        const hasApiChanges = diffEntries.some(entry => entry.content.includes('export') ||
        ...
        const breakingChanges = diffEntries.some(entry => entry.content.includes('BREAKING CHANGE') ||
    ...
    generateChangeSummary(diffEntries, changeType) {
    ...
    buildCommitMessagePrompt(diffEntries, analysis, config) {
...
The commit message should be clear, concise, and follow best practices.`;
    ...
    parseCommitResponse(response) {
        try {
            ...
            const primaryMessage = lines.find(line => line.includes('primary_message') ||
                line.includes('message'))?.replace(/.*[:"]([^"]+)[",].*/, '$1') || 'Update files';
            return {
...
exports.ClaudeCommitClient = ClaudeCommitClient;
