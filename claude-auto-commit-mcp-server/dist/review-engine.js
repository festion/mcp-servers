...
exports.PreCommitReviewEngine = void 0;
const child_process_1 = require("child_process");
class PreCommitReviewEngine {
    config;
    workingDir;
    ...
    async analyzeDiffForProblems(diffEntries) {
        const findings = [];
    ...
    async verifyTaskCompletion(diffEntries) {
        const findings = [];
        const taskFiles = diffEntries.filter(entry => entry.file.includes('tasks.md') ||
        ...
        for (const taskFile of taskFiles) {
            ...
            for (const task of tasks) {
                if (task.completed) {
                    ...
                    const hasImplementation = diffEntries.some(entry => entry.file !== taskFile.file &&
    ...
    extractTasksFromDiff(diffContent) {
        const tasks = [];
    ...
    async checkDocumentationAlignment(diffEntries) {
        const findings = [];
        // Look for documentation files
        const docFiles = diffEntries.filter(entry => entry.file.endsWith('.md') ||
            entry.file.includes('README') ||
            entry.file.includes('docs/'));
        // Look for code files
        const codeFiles = diffEntries.filter(entry => entry.file.endsWith('.js') ||
        ...
        if (codeFiles.length > 0 && docFiles.length === 0) {
            const hasSignificantChanges = codeFiles.some(file => file.additions > 20 || file.status === 'added');
    ...
    detectApiChanges(diffContent) {
        ...
        return apiPatterns.some(pattern => diffContent.split('\n').some(line => line.startsWith('+') && pattern.test(line)));
    }
    async detectFunctionalityRemoval(diffEntries) {
        const findings = [];
        for (const entry of diffEntries) {
    ...
    async assessTestQuality(diffEntries) {
        const findings = [];
        const testFiles = diffEntries.filter(entry => entry.file.includes('test') ||
    ...
    async verifyTestAlignment(diffEntries) {
        const findings = [];
        // This is a simplified version - in practice, you'd need more sophisticated analysis
        const testFiles = diffEntries.filter(entry => entry.file.includes('test') || entry.file.includes('spec'));
        for (const testFile of testFiles) {
    ...
    async analyzeCoverageReduction(diffEntries) {
        const findings = [];
        const testFiles = diffEntries.filter(entry => entry.file.includes('test') || entry.file.includes('spec'));
    ...
    generateReviewSummary(findings, diffEntries) {
...
exports.PreCommitReviewEngine = PreCommitReviewEngine;
