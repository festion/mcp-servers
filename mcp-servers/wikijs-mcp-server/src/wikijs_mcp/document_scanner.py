...
class DocumentScanner:
    ...
    def _validate_search_path(self, search_path: str) -> None:
        ...
        for blocked_path in self.security_config.blocked_paths:
            if abs_search_path.startswith(blocked_path):
                raise SecurityError(f"Search path blocked for security: {search_path}")
    ...
    def _validate_file_path(self, file_path: str) -> None:
        ...
        # Check against blocked paths
        for blocked_path in self.security_config.blocked_paths:
            if abs_file_path.startswith(blocked_path):
                raise SecurityError(f"File path blocked for security: {file_path}")
