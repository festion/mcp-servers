#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Setup script for Network MCP Server - Source to Deployment Installation."""
...
def setup_directories(source_dir=None, install_dir=None):
    ...
    if platform.system() == "Windows":
        default_source = Path("C:/git/mcp-servers/network-mcp-server")
        default_install = Path("C:/working/network-mcp-server")
    else:
        # For Unix systems, use more typical paths
        default_source = Path.cwd()  # Assume we're running from source
        default_install = Path.home() / "mcp-servers" / "network-mcp-server"
    ...
    if not (source_dir / "pyproject.toml").exists() or not (source_dir / "src" / "network_mcp").exists():
        print(f"[ERROR] Invalid source directory: {source_dir}")
        print("Expected network-mcp-server source directory with pyproject.toml and src/network_mcp")
        return None, None
...
def create_sample_config(install_dir):
    ...
    result = run_command(
        f"network-mcp-server create-config \"{config_path}\"",
    ...
    if result is not None:
        ...
        validate_result = run_command(
            f"network-mcp-server validate-config \"{config_path}\"",
...
def show_claude_integration(install_dir):
    ...
    if platform.system() == "Windows":
        script_name = "network-mcp-server.exe"
        scripts_dir = Path(sys.executable).parent / "Scripts"
        script_path = scripts_dir / script_name
    else:
        script_name = "network-mcp-server"
...
def show_next_steps(install_dir):
    """Show configuration and next steps."""
    config_path = install_dir / "config.json"
    
    print(f"\nNext steps:")
    print(f"1. Edit {config_path} with your SMB share details:")
    print("   - host: SMB server hostname or IP address")
    print("   - share_name: Name of the SMB share")
    print("   - username: SMB username")
    print("   - password: SMB password")
    print("   - domain: SMB domain (if required)")
    print("   - port: SMB port (default: 445)")
    print(f"2. Test configuration: network-mcp-server validate-config \"{config_path}\"")
    print(f"3. Test connection: network-mcp-server run --config \"{config_path}\"")
    print("4. Add to Claude Desktop (see configuration above)")
    print("5. Restart Claude Desktop")
    print()
    print("IMPORTANT: Remember to configure your SMB credentials in config.json")
    print("TIP: You can test SMB connectivity before using with Claude Desktop")


def main():
    """Main setup process."""
    parser = argparse.ArgumentParser(description="Network MCP Server Setup (Source to Deployment)")
    parser.add_argument("--source", help="Source directory path")
    parser.add_argument("--install-dir", help="Installation directory path")
    
    args = parser.parse_args()
    
    print("Network MCP Server Setup (Source to Deployment)")
    print("=" * 60)
    ...
    # Set up directories
    source_dir, install_dir = setup_directories(args.source, args.install_dir)
    if not source_dir or not install_dir:
        sys.exit(1)
    
    # Interactive install directory selection if not specified
    ...
    print("\n[INFO] Installing Network MCP Server...")
    ...
    if run_command("network-mcp-server --help", "Testing network-mcp-server command", check=False):
        print("[SUCCESS] network-mcp-server command is available")
    else:
        print("[WARNING] network-mcp-server not in PATH, trying alternative...")
        if run_command("python -m network_mcp.cli --help", "Testing alternative command", check=False):
            print("[SUCCESS] Can run via: python -m network_mcp.cli")
        else:
            print("[ERROR] Cannot run network-mcp-server")
            sys.exit(1)
    ...
    if create_sample_config(install_dir):
        ...
        print("[WARNING] Configuration setup failed, but installation can continue")
    ...
    print(f"[SUCCESS] Network MCP Server installed to: {install_dir}")
    print(f"[SUCCESS] Configuration created: {install_dir}/config.json")
